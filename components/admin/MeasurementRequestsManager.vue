<template>
  <div>
    <div class="mb-6">
      <h2 class="text-xl font-bold text-gray-900">Заказы замеров</h2>
    </div>

    <!-- Measurement Requests List -->
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="request in measurementRequests" :key="request.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ request.address }}</td>
      ...
      <div class="bg-white rounded-lg max-w-2xl w-full">
        <div class="p-6">
          <div class="flex justify-between items-center mb-6">
            <h3 class="text-lg font-medium text-gray-900">Детали заказа замера</h3>
            <button
          ...
          <div class="grid grid-cols-2 gap-6 mb-6">
            ...
            <div class="col-span-2">
              ...
              <p class="mt-1">{{ selectedRequest.address }}</p>
            </div>
            <div class="col-span-2">
...
const measurementRequests = ref<MeasurementRequest[]>([])
const selectedRequest = ref<MeasurementRequest | null>(null)

const loadMeasurementRequests = async () => {
  try {
    const token = localStorage.getItem('admin_token')
    const data = await api.get('/api/measurement-requests', {
      headers: {
        'Authorization': `Bearer ${token}`,
      }
    })
    measurementRequests.value = data.map((request: MeasurementRequest) => {
...
const updateRequestStatus = async (request: MeasurementRequest, event: Event) => {
  try {
    ...
    await api.put(`/api/measurement-requests/${request.id}/status`, { status: request.status }, { headers })
  } catch (error) {
    console.error('Error updating measurement request status:', error)
  }
}

const viewRequestDetails = (request: MeasurementRequest) => {
